...
class TimelineView: UIView, UIScrollViewDelegate {
    ...
    let scroller = TimelineScroller()//->缩略图和时间刻度
    ...
    override init (frame: CGRect) {
        ...
        //mod
        for trimView in scroller.trimViews {
            self.addSubview(trimView)
        }
        self.addSubview(centerLine)
        centerLine.layer.zPosition = 1000
    ...
    func newMovieSet() {
        ...
        if let asset = mainView!.asset{
            ...
            //mod
            for trimView in scroller.trimViews {
                trimView.reset(duration:durationFloat)
            }
        }
    }
    ...
    func setTrimmerStatus(enabled:Bool) {
        ...
        for trimView in scroller.trimViews {
            if enabled {
                trimView.alpha = 1
                trimView.startKnob.isUserInteractionEnabled = true
                trimView.alpha = 1
                trimView.endKnob.isUserInteractionEnabled = true
                
            } else {
                trimView.alpha = 0.5
                trimView.startKnob.isUserInteractionEnabled = false
                trimView.alpha = 0.5
                trimView.endKnob.isUserInteractionEnabled = false
            }
        }
    }
...
//        scroller.trimView.moveToTimeAndTrimWithAnimation(time, trim:trim)
    ...
    func setTrimViewInteraction(_ active:Bool) {
        ...
        for trimView in scroller.trimViews {
            trimView.startKnob.isUserInteractionEnabled = active
            trimView.endKnob.isUserInteractionEnabled = active
        }
        if active {
...
//            }
//        }
        return (0.0, mainView!.duration)
    }
    

    func swapTrimKnobs() {
        for trimView in scroller.trimViews {
            let knob = trimView.endKnob
           trimView.endKnob = trimView.startKnob
            trimView.startKnob = knob
        }
    }
    ...
    func endPinch() {
        ...
        UIView.animate(withDuration: 0.1,delay:Double(0.0),options:UIView.AnimationOptions.curveEaseOut, animations: { () -> Void in
            ...
            for trimView in self.scroller.trimViews {
                trimView.layout()
            }
